...
trait HasSidebar
{
    ...
    protected function resolveCurrentUser(): ?Scribe
    {
    ...
    protected function computeSubscriptionLabel(Model $record): string
    {
        ...
        if (! $user) {
            return $this->evaluateSubscriptionOverride($this->subscribeLabelOverride, $record) ?? __('notations::notes.subscribe');
        }

        if ($record->isSubscribed($user)) {
            return $this->evaluateSubscriptionOverride($this->unsubscribeLabelOverride, $record) ?? __('notations::notes.unsubscribe');
        }
