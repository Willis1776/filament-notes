...
test('it cannot save a comment when the policy denies it', function () {
    ...
    \Gate::policy(Note::class, \Tests\Policies\BlockedNotePolicy::class);
...
test('it dispatches events for subscribers (distinct event)', function () {
    ...
    Event::assertDispatched(UserIsSubscribedToNotableEvent::class, function ($event) use ($subscriber, $comment) {
        return $event->user->is($subscriber) && $event->comment->is($comment);
    });

    Event::assertNotDispatched(UserIsSubscribedToNotableEvent::class, function ($event) use ($author) {
