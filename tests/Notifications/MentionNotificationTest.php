...
test('it sends a mention notification when enabled with configured channels', function () {
    ...
    NotificationFacade::assertSentTo(
        $mentioned,
        UserMentionedInNote::class,
        function (UserMentionedInNote $notification, array $channels) use ($comment) {
...
{
    public function __construct(public Note $comment, public array $channels) {}
...
test('it uses the configured comment URL resolver in notification payload', function () {
    ...
    Config::resolveCommentUrlUsing(function (Note $comment) use ($resolvedUrl) {
        return $resolvedUrl;
    });
    ...
    NotificationFacade::assertSentTo(
        $mentioned,
        UserMentionedInNote::class,
        function (UserMentionedInNote $notification) use ($resolvedUrl, $mentioned) {
