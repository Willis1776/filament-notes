...
```bash
composer require willis1776/notes
```
...
class Project extends Model implements Notable
{
    use HasNotes;
}
```
...
```php
use Kirschbaum\Commentions\Filament\Infolists\Components\NotesEntry;

Infolists\Components\Section::make('Comments')
    ->schema([
        NotesEntry::make('comments')
            ->mentionables(fn (Model $record) => User::all())
            ->perPage(8)
...
use Kirschbaum\Commentions\Contracts\Commenter;
use Kirschbaum\Commentions\Policies\NotePolicy as CommentionsPolicy;
...
class SendSubscribedUserNotification implements ShouldQueue
{
    ...
    public function handle(UserIsSubscribedToNotableEvent $event): void
    {
        $event->user->notify(
...
Config::resolveCommentUrlUsing(function (\Kirschbaum\Commentions\Note $comment) {
...
{
    $statusHistory = $this->statusHistory()->get()->map(fn (StatusHistory $statusHistory) => new RenderableNote(
        id: $statusHistory->id,
